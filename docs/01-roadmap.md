...
**Phase 157**: ✅ Summary JSON 補正 + 互先/ハンデ分離 + 軽微整理（2026-06-28 完了）。

ユーザー依頼（2026-06-28 メモ）に基づき、野狐 SGF 解析時に判明した 4 つの問題に対応。schema 3.3 → 3.4。

| Phase | 主題 | 概要 | Lv | 状態 |
|------:|------|------|:--:|:----:|
| **157-A** | summary `loss_progression` バケット不連続バグ修正 | `compute_loss_progression()` に `truncate_end_move` フラグ追加、Summary 集計時は False で呼び出し | Lv1 | ✅ (2026-06-28) |
| **157-B** | rank_classifier 中国語対応（段/级） | regex `[段级]` 追加、`fox sgf works.sgf` の `BR[15级]` 等が有効に | Lv1 | ✅ (2026-06-28) |
| **157-D** | top-level `win_loss_analysis` null 削除 | schema から該当フィールド削除、`win_loss_analysis` は per-player のみ | Lv0 | ✅ (2026-06-28) |
| **157-C** | summary 互先/ハンデ分離出力 | `game_classifier.py` 新設、`loss_progression` を dict 化、per-player `even`/`handicapped` ブロック追加 | Lv2 | ✅ (2026-06-28) |
| **157-E** | 時間プレッシャー分析の調査メモ | `docs/ideas/time_pressure_analysis.md` 作成（実装なし） | Lv0 | ✅ (2026-06-28) |

#### 157-A バグ詳細

`summary_json_export.py` で `(b.start_move, b.end_move)` を集計キーにしていたが、`b.end_move` は局ごとに末尾で切り詰められていたため、局A 186手 → `(181, 186)`、局B 195手 → `(181, 190)` が別バケットとして並ぶ不連続な progression になっていた。
修正: `compute_loss_progression(truncate_end_move=False)` で正規 end に揃え、Karte（per-game）は従来通り `True`。

#### 157-B 仕様変更

`rank_classifier.py` の正規表現を `^\s*(\d{1,2})\s*([kKdD])\s*$` → `^\s*(\d{1,2})\s*([kKdD段级])\s*$` に拡張。これにより foxwq の `BR[4段] WR[4段]` が `dan` バケットに、QQ Go の `BR[15级]` が `kyu` バケットに分類される。テスト `test_rank_classifier.py` の `("5段", None)` → UNKNOWN 期待値を `("5段", "DAN")` に更新。

#### 157-C スキーマ変更点（3.3 → 3.4）

| フィールド | 変更内容 |
|-----------|---------|
| `SummaryReport.loss_progression` | `List[Dict]` → `Dict[str, List[Dict]]` (`"all"`, `"even"`, `"handicapped"` の3系統) |
| `SummaryReport.win_loss_analysis` | **削除**（Phase 157-D） |
| `SummaryPlayerStats.even` | 追加（`NotRequired`）。even 局が無い時は省略 |
| `SummaryPlayerStats.handicapped` | 追加（`NotRequired`）。handicap 局が無い時は省略 |
| `MetaData.games_by_type` | 追加（`Optional[Dict[str, int]]`）。Karte では null |
| `REPORT_SCHEMA_VERSION` | "3.3" → "3.4" |

#### 157-C game_classifier 分類ルール

| 状態 | 条件 |
|------|------|
| `even` | `handicap == 0 AND komi >= 6.0`（コミ 6.5 / 7.5 を含む） |
| `handicapped` | `handicap >= 2 OR komi < 6.0`（置き石 or コミなし／減コミ） |
| `unknown` | いずれにも該当しない（稀：HA=0 で 0 < KM < 6 など） |

#### 新規ファイル

| パス | 役割 |
|------|------|
| `katrain/core/reports/utils/game_classifier.py` | 局の even/handicapped 分類、`classify_games()` |
| `docs/ideas/time_pressure_analysis.md` | 157-E の調査メモ |
| `tests/test_game_classifier.py` | game_classifier のユニットテスト |

#### 変更ファイル

| パス | 変更 |
|------|------|
| `katrain/core/reports/utils/loss_progression.py` | `truncate_end_move` パラメータ追加 |
| `katrain/core/reports/utils/rank_classifier.py` | regex に `[段级]` 追加 |
| `katrain/core/reports/utils/__init__.py` | `game_classifier` をエクスポート |
| `katrain/core/reports/schema.py` | `SummaryReport` の `win_loss_analysis` 削除、`even`/`handicapped`/`games_by_type` 追加、`loss_progression` を dict 化 |
| `katrain/core/reports/definitions.py` | `REPORT_SCHEMA_VERSION` 3.3 → 3.4 |
| `katrain/core/reports/summary_json_export.py` | `_build_player_stats_block()` / `_compute_loss_progression_block()` ヘルパー抽出、3 系統出力対応、top-level `win_loss_analysis` 削除 |
| `tests/fixtures/golden/karte_sgf_fox.golden` | `opponent_strength_loss_correlation` の `no_opponent_info` → `insufficient_data`（kyu バケット 1 件計上）に更新 |
| `tests/test_loss_progression.py` | `truncate_end_move` True/False テスト追加（+5 件） |
| `tests/test_rank_classifier.py` | 中国語ケース追加（+8 件）、`"5段"` を UNKNOWN → DAN に期待値更新 |
| `tests/test_opponent_analysis.py` | 中国語 rank で `computed` を返すテスト追加（+3 件） |
| `tests/test_batch_json.py` | schema_version 3.4 更新、157-C/D 用テスト追加（+4 件） |

#### 残オープン項目

- GUI での `even` / `handicapped` 切替 UI（Phase 157-C の Config 化は今回スキップ）
- `loss_progression` の bucket_size Config 化（Phase 154 残オープン）
- 漢数字段級位（"五段" 等）（Phase 157-B の次）
- 時間プレッシャー分析（157-E → 将来フェーズ）
...
- [x] **Phase 207-213: `core/coach/` パッケージ完全実装**（Lv2 + Lv3、2026-07-14 完了、合計 4752 件テスト合格）
  ...
  - **Phase 270 (2026-07-18)**: 複数カルテ集約 + サマリプロンプト v3.5 拡張. 単局カルテが持つ拡張フィールド (`area` / `position_difficulty` / `meaning_tag_label` / `reason_tags_distribution` / `data_quality` / loss-progression spikes) を、`build_summary_json` 経由では失われていた集約サマリ JSON に復活。`katrain/core/coach/karte_aggregator.py` 新設 + 6 集約関数 (`aggregate_reason_tags_by_color` / `aggregate_area_difficulty` / `detect_loss_spike_windows` / `group_representative_moves_by_tag` / `aggregate_data_quality` / `build_meaning_tag_label_map`) + `SummaryPromptConfig.kartes` 経由でオプトイン的に組み込み + Schema 3.5 への条件付きバンプ。既存 3.4 経路は完全後方互換 (4 ファイル変更 / 1 ファイル新規 / +52 unit tests、合計 5,667 件テスト合格)。
...
- [x] **Phase 219: Calibrate CLI command**（Lv1、2026-07-14 完了）
