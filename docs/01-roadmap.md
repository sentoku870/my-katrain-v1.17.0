...
- `loss_progression` の bucket_size はデフォルト 10（固定）。Config 経由で変更可能な仕様は将来検討
...
| **155** | 相手の棋力との相関 | `rank_black/white` を `GameSummaryData` 追加、級位/有段/高段バケット変換 | Lv3 | 計画中 |
| **156** | 動的フェーズ分割（scoreStdev ベース） | `classify_game_phase_dynamic()` 関数新設（オプトイン） | Lv4 | 計画中 |
