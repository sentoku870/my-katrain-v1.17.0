...
class TestPhase170PlayMethodsRemoved:
    """Phase 170: ``play_move`` / ``request_move`` / ``_PLAY_COLOR_MAP`` removed.

    The human-vs-Leela play path used to live on ``LeelaEngine`` as
    thin GTP wrappers around ``play`` and ``genmove``. Phase 170
    abolished the play feature, so those methods are gone. The test
    here documents that removal so any regression that re-introduces
    them surfaces immediately.
    """

    def test_play_methods_no_longer_exist(self) -> None:
        from katrain.core.leela import engine as engine_module

        assert not hasattr(engine_module.LeelaEngine, "play_move"), (
            "Phase 170 removed LeelaEngine.play_move; if you re-add it, "
            "update the tests and explain why Leela play is back."
        )
        assert not hasattr(engine_module.LeelaEngine, "request_move"), (
            "Phase 170 removed LeelaEngine.request_move; if you re-add it, "
            "update the tests and explain why Leela play is back."
        )
        assert not hasattr(engine_module.LeelaEngine, "_PLAY_COLOR_MAP"), (
            "Phase 170 removed LeelaEngine._PLAY_COLOR_MAP alongside the play wrappers."
        )
