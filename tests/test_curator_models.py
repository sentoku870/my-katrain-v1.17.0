...
(needs_match), meaning-tag-based scoring fields, and UNCERTAIN_TAG have been
removed.

Data models for suitability scoring:
- SuitabilityConfig (frozen dataclass with stability-only config)
...
class TestSuitabilityConfig:
    ...
    def test_default_values(self):
        """Default config has expected values per Phase 137+ docs."""
    ...
    def test_frozen_dataclass(self):
        ...
        with pytest.raises((AttributeError, Exception)):
            cfg.max_volatility = 5.0  # type: ignore[misc]
...
class TestSuitabilityScore:
    ...
    def test_debug_info_must_be_mapping_or_none(self):
        ...
        score = SuitabilityScore(stability=0.5, total=0.5, debug_info=None)
...
class TestEqualityAndHashing:
    ...
    def test_suitability_config_equality(self):
        ...
        c1 = SuitabilityConfig(max_volatility=10.0, stability_insufficient_data=0.5)
        c2 = SuitabilityConfig(max_volatility=10.0, stability_insufficient_data=0.5)
        assert c1 == c2

    def test_suitability_config_inequality(self):
        """Different configs compare unequal."""
        c1 = SuitabilityConfig(max_volatility=10.0)
        c2 = SuitabilityConfig(max_volatility=20.0)
        assert c1 != c2

    def test_suitability_score_equality(self):
        """Two scores with same fields compare equal."""
        s1 = SuitabilityScore(stability=0.5, total=0.5, percentile=50)
        s2 = SuitabilityScore(stability=0.5, total=0.5, percentile=50)
        assert s1 == s2

    def test_suitability_score_hashable(self):
        """Frozen dataclasses are hashable when debug_info is None."""
        s1 = SuitabilityScore(stability=0.5, total=0.5)
        s2 = SuitabilityScore(stability=0.5, total=0.5)
