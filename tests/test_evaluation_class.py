...
class TestIndexBounds:
    ...
    def test_index_in_bounds(self, points_lost: float) -> None:
        ...
        assert 0 <= idx < NUM_BUCKETS, f"points_lost={points_lost} → idx={idx}, expected [0, {NUM_BUCKETS - 1}]"
...
class TestExactMapping:
    ...
    def test_exact_mapping(self, points_lost: float, expected: int) -> None:
        ...
        assert actual == expected, f"points_lost={points_lost}: expected idx={expected}, got {actual}"
...
class TestMonotonicity:
    ...
    def test_monotonicity(self) -> None:
        ...
        losses = [-10.0, -1.0, 0.0, 0.3, 0.5, 1.0, 1.5, 2.0, 3.0, 6.0, 12.0, 20.0, 100.0]
        indices = [evaluation_class(loss, DESCENDING_THRESHOLDS) for loss in losses]

        for i in range(len(indices) - 1):
            assert indices[i] >= indices[i + 1], (
                f"Monotonicity violation: loss={losses[i]}→idx={indices[i]}, loss={losses[i + 1]}→idx={indices[i + 1]}"
            )
...
class TestAllBucketsReachable:
    ...
    def test_all_buckets_reachable(self) -> None:
        ...
        for expected_bucket, loss in representative.items():
            ...
            assert actual == expected_bucket, f"Bucket {expected_bucket} not reachable: loss={loss} → idx={actual}"
...
class TestNoneThresholds:
    ...
    def test_bounds_with_none(self) -> None:
        ...
        for loss in [-10.0, 0.0, 1.0, 5.0, 15.0]:
            ...
            assert 0 <= idx < len(thresholds), f"With None entries: loss={loss} → idx={idx}, len={len(thresholds)}"
...
class TestEdgeCases:
    ...
    def test_negative_loss_gains(self) -> None:
        ...
        for loss in [-0.5, -1.0, -5.0, -100.0]:
            ...
            assert idx >= 4, f"Negative loss (gain) {loss} should map to high bucket, got {idx}"
...
class TestAscendingThresholds:
    ...
    def test_ascending_thresholds_still_work(self) -> None:
        ...
        for loss in [-10.0, 0.0, 1.0, 5.0, 15.0]:
            ...
            assert 0 <= idx < len(ascending), f"Ascending thresholds: loss={loss} → idx={idx}, len={len(ascending)}"
