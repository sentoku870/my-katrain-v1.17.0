...
class LeelaEngine:
    ...
    #   with the surrounding KataGo Game.
    # - ``genmove <color>``: ask Leela to generate the next move for
    #   ``<color>``. Returns a single GTP ``= <coord>`` (or ``= resign``)
    #   line.
    #
    # The methods below are intentionally simple: they wrap the existing
    # ``_send_command`` / ``set_position`` machinery so the analysis and
    # play paths share the same engine-process state. A real
    # production-grade integration would also expose ``undo``,
    # ``time_settings``, ``kata-set-rules`` etc.; this minimal surface is
    # enough to drive ``LeelaStrategy`` end-to-end (human-vs-Leela).

    _PLAY_COLOR_MAP: dict[str, str] = {"B": "black", "W": "white", "BLACK": "black", "WHITE": "white"}

    def play_move(self, color: str, coord: str) -> bool:
        """Send a human move to Leela via ``play`` (Phase 159B).

        Args:
            color: Internal color token (``"B"`` / ``"W"``).
            coord: GTP coordinate (e.g. ``"D4"`` or ``"pass"``).

        Returns:
            True on successful send; False if the engine is not alive.
        """
        if not self.is_alive() and not self.start():
            return False
        gtp_color = self._PLAY_COLOR_MAP.get(color.upper(), "black")
        ok = self._send_command(f"play {gtp_color} {coord}")
        if ok:
            with self._lock:
                self._moves.append((color, coord))
        return ok
    ...
    def request_move(
        ...
        """Ask Leela for the next move via ``genmove`` (Phase 159B).

        Args:
            color: Internal color token (``"B"`` / ``"W"``) — whose turn
                it is to play.
            callback: Called with the move coord (e.g. ``"D4"`` or
                ``"pass"``) on success, or with an empty string on failure.
            ...
            block: When ``True`` (default) the call runs synchronously on
                ...
                background thread (mirroring ``request_analysis``) and
                returns immediately. The 159B ``LeelaStrategy`` path uses
                the synchronous variant because KataGo's AI loop expects
                a deterministic move in its turn.

        Returns:
            True if the request was dispatched; False if the engine is
            not alive.
        """
        if not self.is_alive() and not self.start():
            return False
        ...
        def _run_genmove() -> None:
            try:
                self._send_command(f"genmove {gtp_color}")
                with self._lock:
                    proc = self.process
                ...
                # ``genmove`` is synchronous in GTP — it returns one line
                # starting with ``=``. We block on readline with a
                # timeout-friendly loop and break out as soon as we see
                # the response.
                timeout_s = min(60.0, max(1.0, visits / 50.0))
                start = time.time()
                line = ""
                while time.time() - start < timeout_s:
                    ...
                    if not new_line:
                        ...
                        break
                    line = new_line.strip()
                    if line.startswith(self.GTP_SUCCESS) or line.startswith(self.GTP_FAILURE):
                        break

                if not line.startswith(self.GTP_SUCCESS):
                    callback("")
                    return

                # ``= <coord>`` (strip the success marker)
                payload = line[len(self.GTP_SUCCESS):].strip()
                if not payload:
                    callback("")
                    return
                if payload.lower() == "resign":
                    callback("resign")
                    return
                callback(payload)
            except Exception as e:
                logger.exception(f"request_move error: {e}")
                callback("")

        if block:
            _run_genmove()
        else:
            threading.Thread(target=_run_genmove, daemon=True).start()
        return True
