...
class ProgressLoader(BoxLayout):
    ...
    def __init__(self, root_instance: Any, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.root_instance = root_instance
        self.request = None
        Clock.schedule_once(self.start, 0)

    def start(self, _dt: float) -> None:
    ...
    def animation_show(self, _dt: float) -> None:
        animation = Animation(opacity=1, d=0.2, t="out_quad")
        animation.start(self)

    def request_download_file(self, url: str, path: str) -> None:
        """
        :type url: str;
    ...
    def handle_error(self, request: Any, error: str) -> None:
    ...
    def update_progress(self, request: Any, current_size: int, total_size: int) -> None:
        if total_size < 1e4:
            current_size = 0
        percent = current_size / max(total_size, 1)
        self.label_downloading_text = self.downloading_text.format(f"{percent:.1%}")

    def handle_success(self, request: Any, result: Any) -> None:
        self.cleanup()
